...
-- Tools that should NEVER be equipped/activated by any automation
...
-- Searches Workspace.buyprompts.mix for a named part across all children.
-- Used for shops that may appear in multiple folders (e.g. two cart shops).
local function FindInMix(partName)
    local mix = SafeGet(function() return Workspace.buyprompts.mix end)
    if not mix then return nil end
    for _, shop in pairs(mix:GetChildren()) do
        for _, v in pairs(shop:GetDescendants()) do
            if v.Name == partName and v:IsA("BasePart") then
                return v
            end
        end
    end
...
local function DisableHitboxExtender()
    ...
    for _, conn in pairs(hitboxExtenderConnections) do
        if type(conn) == "table" then
            for _, subConn in pairs(conn) do pcall(function() subConn:Disconnect() end) end
        else
...
-- =====================
-- AIMBOT
-- mousemoverel relative delta, falls back to VirtualInputGame
-- =====================

local function GetClosestPlayer()
    local hrp = GetHRP()
    if not hrp then return nil end
    local closestPlayer, closestDistance = nil, aimbotRange
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            local character = player.Character
...
local function MoveMouseRel(deltaX, deltaY)
    if mousemoverel then
        ...
        pcall(function()
            ...
            VirtualInput:SendMouseMoveEvent(Vector2.new(mouse.X + deltaX, mouse.Y + deltaY))
        end)
    end
end
...
-- =====================
-- F KEY — GRENADES/BOMBS ONLY
-- Only equips and activates tools whose name contains "grenade" or "bomb"
-- =====================

local function GetGrenadeTools()
    local char = GetChar()
    local tools = {}
    local function checkTool(t)
        if not t:IsA("Tool") then return end
        if not t:FindFirstChild("Handle") then return end
        local name = t.Name:lower()
        if name:find("grenade") or name:find("bomb") then
            table.insert(tools, t)
        end
    end
    if char then
        for _, child in pairs(char:GetChildren()) do checkTool(child) end
    end
    for _, child in pairs(LocalPlayer.Backpack:GetChildren()) do checkTool(child) end
    return tools
end

local function EquipAndThrowGrenades()
    if isAttacking then return end
    isAttacking = true
    local char = GetChar()
    if not char then isAttacking = false return end

    local tools = GetGrenadeTools()
    if #tools == 0 then
        Rayfield:Notify({ Title = "Grenades", Content = "No grenades/bombs found in inventory.", Duration = 3, Image = "alert-circle" })
        isAttacking = false
        return
    end

    -- Move all grenades to character first
    for _, tool in ipairs(tools) do
        pcall(function() tool.Parent = char end)
    end
    task.wait(0.05)

    -- Disconnect old ancestry watchers
    for _, conn in ipairs(equipAllConnections) do
        pcall(function() conn:Disconnect() end)
    end
    equipAllConnections = {}

    -- Keep grenades in hand if game tries to unequip them
    for _, tool in ipairs(tools) do
    ...
    -- Equip and activate each grenade in parallel
    for _, tool in ipairs(tools) do
        task.spawn(function()
            pcall(function()
                char.Humanoid:EquipTool(tool)
    ...
    Rayfield:Notify({ Title = "Grenades", Content = "Threw " .. #tools .. " grenade(s)!", Duration = 2, Image = "zap" })
    task.wait(0.1)
    isAttacking = false
end
...
Tab2:CreateButton({
    ...
    Callback = function()
        ...
        -- Path: Workspace.buyprompts.mix["hospital shop"].buypromptbag
        local promptPart = SafeGet(function()
            return Workspace.buyprompts.mix["hospital shop"].buypromptbag
        end)
        if not promptPart then
            Rayfield:Notify({ Title = "Fast Buy", Content = "Hospital shop not found in mix.", Duration = 3, Image = "alert-circle" })
            return
        end
        local prompt = promptPart:FindFirstChildWhichIsA("ProximityPrompt")
        if not prompt then
        ...
        TeleportDoReturn(CFrame.new(promptPart.Position) + Vector3.new(0, 3, 0), function()
            pcall(function() fireproximityprompt(prompt) end)
        end, 0.3)
    end
})
...
Tab4:CreateParagraph({
    Title = "Warning",
    Content = "Spends all your shillings on grenades. Searches both cart shops in buyprompts.mix for buypromptF."
})
...
Tab4:CreateButton({
    ...
    Callback = function()
        ...
        -- Search both cart shops inside buyprompts.mix for buypromptF
        local promptPart = FindInMix("buypromptF")
...
Tab6:CreateButton({
    Name = "Throw Grenades (Same as F Key)",
    Callback = function()
        EquipAndThrowGrenades()
    end
})
